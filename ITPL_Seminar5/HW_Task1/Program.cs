...
void PrintElementByPosition(int[,] array, int x, int y, bool inArray)
{
    if (inArray)
    {
        Console.Write($"{array[x - 1, y - 1], 2} ");
    }
...
bool inArray = ValidatePosition(array, x, y);
PrintElementByPosition(array, x, y, inArray);
//}


/* ПРИМЕР РЕШЕНИЯ ИЗ АВТОТЕСТА на GeekBrains
Меньше переменных и код короче, а значит
